...
onEvent('server.tick', event => {
  if (Date.now() > nextHour) {
    ...
    event.server.runCommandSilent(`execute in minecraft:overworld season set ${constants.formatSerene.apply(constants, babelHelpers.toConsumableArray(data))}`); // event.server.runCommand(`betterweather setseason ${constants.formatBW(...data)}`)
  }
});