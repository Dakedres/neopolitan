...
onEvent('item.tags', event => {
  var equalMetal = (left, right) => [[`forge:nuggets/${left}`, `#forge:nuggets/${right}`], [`forge:ingots/${left}`, `#forge:ingots/${right}`], [`forge:plates/${left}`, `#forge:plates/${right}`]].map(a => event.add.apply(event, babelHelpers.toConsumableArray(a)));

  var addFrom = function (mod) {
  ...
  ['kubejs:crushed_sunmetal', 'kubejs:crushed_steel'].forEach(i => event.add('create:crushed_ores', i)); // event.add('supplementaries:sign_post', materialFrom('supplementaries', 'sign_post') )
